...
MetaBot 是一个 TypeScript ESM 项目，连接 IM 平台（飞书、Telegram）与 Codex Agent SDK。
...
| `src/codex/executor.ts` | 封装 Agent SDK 的 `query()` 为异步生成器。 |
| `src/codex/stream-processor.ts` | 将 SDK 消息转为卡片状态对象。 |
| `src/codex/session-manager.ts` | 内存中的会话存储，按 `chatId` 索引。24 小时过期。 |
