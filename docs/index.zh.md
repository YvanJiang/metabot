...
Codex 是最强的 AI 编码 Agent —— 但它被锁在笔记本终端里。

MetaBot 解放了它。给每个 Agent 一个 Codex 大脑、持久化的共享记忆、创建新 Agent 的能力、以及通信总线。全部可以从飞书或 Telegram 手机端控制。

## 核心组件

| 组件 | 说明 |
|------|------|
| **Codex 内核** | 每个 Bot 都是完整的 Codex 实例 — Read, Write, Edit, Bash, Glob, Grep, WebSearch, MCP 等。`bypassPermissions` 模式自主运行。 |
| **MetaSkill** | Agent 工厂。`/metaskill ios app` 调研最佳实践后生成完整的 `.codex/` Agent 团队。 |
...
| **Agent 总线** | 9100 端口 REST API。Agent 通过 `cb talk` 互相对话。运行时创建/删除 Bot。 |
| **Peers 联邦** | 跨实例 Bot 发现和任务路由。 |
| **定时任务调度器** | 一次性延迟和周期性 cron 任务。支持时区，跨重启持久化。 |
| **CLI 工具** | `codexbot`、`mm`、`cb`、`fd` 命令，管理服务、知识库、Agent 总线和飞书文档。 |
...
安装器引导：工作目录 → Codex 认证 → IM 凭证 → PM2 自动启动。
