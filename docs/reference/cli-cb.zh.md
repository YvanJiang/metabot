...
`cb` 命令提供终端访问 MetaBot Agent 总线 API。

## 安装

MetaBot 安装器自动安装到 `~/.local/bin/cb`。
...
```bash
cb bots                             # 列出所有 Bot（本地 + peer）
cb bot <name>                       # 获取 Bot 详情
```
...
cb talk alice/bot <chatId> <prompt> # 指定 peer 的 Bot 对话
```
...
cb schedule add <bot> <chatId> <delayMs> <prompt>            # 创建一次性任务
cb schedule pause <id>                                        # 暂停
cb schedule resume <id>                                       # 恢复
cb schedule cancel <id>                                       # 取消
```
...
cb voice "你好" --provider doubao    # 指定 TTS 服务商
cb voice "你好" --voice nova         # 指定声音
```

| 参数 | 说明 |
|------|------|
| `--play` | 生成后播放（macOS: afplay, Linux: mpv/ffplay/play） |
| `-o FILE` | 保存到指定文件（默认: `/tmp/cb-voice-<时间戳>.mp3`） |
