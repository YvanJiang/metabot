...
**在飞书 / Telegram / 微信上用手机控制 Codex — 写代码、管 Agent、自动化一切。**
...
安装器引导一切：工作目录 → Codex 认证 → IM 平台 → PM2 自动启动。**5 分钟上手。**

---

## 你能用它做什么

- **手机写代码** — 地铁上用飞书给 Codex 发消息，它帮你改 bug、提 PR、跑测试
...
| | MetaBot | 直接用 Codex | Dify / Coze |
|---|---|---|---|
| **手机控制** | 飞书/TG/微信随时随地 | 只能在终端 | 有，但不能跑代码 |
| **代码能力** | 完整 Codex（Read/Write/Edit/Bash/MCP） | 完整 | 无，只能调 API |
...
```
飞书/TG/微信 → IM Bridge → Codex Agent SDK → 流式卡片更新
                              ↕
                    MetaMemory（共享知识库）
                    MetaSkill（Agent 工厂）
                    定时调度器（Cron 任务）
...
| 组件 | 一句话说明 |
|------|-----------|
| **Codex 内核** | 每个 Bot 都是完整的 Codex — Read/Write/Edit/Bash/Glob/Grep/WebSearch/MCP，`bypassPermissions` 全自动 |
...
支持的消息类型：文本、图片（Codex 多模态）、文件（PDF/代码/文档）、富文本（Post 格式）、多文件批量。
...
|------|------|--------|------|
| `name` | 是 | — | Bot 标识名 |
| `defaultWorkingDirectory` | 是 | — | Codex 的工作目录 |
...
| `model` | 否 | SDK 默认 | Codex 模型 |
| `apiKey` | 否 | — | OpenAI API Key（不设则从 `~/.codex/.credentials.json` 动态读取，兼容 cc-switch） |
...
请使用 `codex login` 或配置 OpenAI API Key。
...
MetaBot 以 `bypassPermissions` 模式运行 Codex — 无交互式确认：

- Codex 对工作目录有完整读写执行权限
...
> **模型切换**：每个会话可独立设置模型。在模型名后加 `[1m]` 可启用 1M 上下文窗口（仅 Opus 4.7/4.6、Sonnet 4.6 支持），例如 `/model codex-opus-4-7[1m]`。OAuth/Pro-Max 登录用户 SDK 会丢弃 beta flag，`[1m]` 后缀是唯一可靠的 1M 开启方式。
...
前置条件：Node.js 20+，[Codex CLI](https://github.com/anthropics/codex-code) 已安装并认证。
