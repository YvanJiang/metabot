---
name: voice
description: Convert text to speech audio using cb voice CLI. Use when the user asks you to speak, say something aloud, generate audio, or produce a voice recording.
---

## Text-to-Speech (Voice Output)

Generate MP3 audio from text using the `cb voice` CLI.

### Quick Commands

```bash
# Generate MP3, prints file path to stdout
cb voice "Hello, this is a test"

# Generate and play immediately
cb voice "Hello" --play

# Save to specific file
cb voice "Hello" -o greeting.mp3

# Override provider and voice
cb voice "Hello" --provider doubao --voice zh_female_wanqudashu_moon_bigtts

# Pipe text (useful for long content)
echo "Long text here" | cb voice
echo "Long text" | cb voice -o output.mp3
```
...
- For short text (greetings, alerts), use inline: `cb voice "text"`
- For longer text, pipe through stdin: `echo "..." | cb voice`
...
- To send the audio to the user in Feishu, copy the file to the outputs directory:
  `cp /tmp/cb-voice-xxx.mp3 /tmp/metabot-outputs/<chatId>/`
