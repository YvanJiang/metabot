...
**Control Codex from your phone via Feishu / Telegram / WeChat — write code, manage agents, automate everything.**
...
The installer walks you through everything: working directory → Codex auth → IM platform → auto-start with PM2. **5 minutes to get started.**

---

## What You Can Build

- **Code from your phone** — message Codex from Feishu on the subway, it fixes bugs, opens PRs, runs tests
...
| | MetaBot | Codex (terminal) | Dify / Coze |
|---|---|---|---|
| **Mobile access** | Feishu/TG/WeChat anywhere | Terminal only | Yes, but can't run code |
| **Code capabilities** | Full Codex (Read/Write/Edit/Bash/MCP) | Full | None, API calls only |
...
```
Feishu/TG/WeChat → IM Bridge → Codex Agent SDK → Streaming card updates
                                    ↕
...
|-----------|-------------|
| **Codex Kernel** | Every bot is a full Codex instance — Read/Write/Edit/Bash/Glob/Grep/WebSearch/MCP, `bypassPermissions` for autonomous operation |
...
Supported: text, images (Codex multimodal), files (PDF/code/docs), rich text (Post format), batch upload.
...
Use `codex login` or configure an OpenAI API key instead.
...
MetaBot runs Codex in `bypassPermissions` mode — no interactive approval:

- Codex has full read/write/execute access to the working directory
...
> **Model switching**: Each session can pick its own model. Append `[1m]` to the model name to enable the 1M context window (only Opus 4.7/4.6 and Sonnet 4.6 support it), e.g. `/model codex-opus-4-7[1m]`. OAuth/Pro-Max users must use this suffix — the SDK silently drops beta headers under that auth mode.
...
Prerequisites: Node.js 20+, [Codex CLI](https://github.com/anthropics/codex-code) installed and authenticated.
